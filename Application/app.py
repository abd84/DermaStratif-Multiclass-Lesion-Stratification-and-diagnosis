...
@app.route('/')
def health():
    return jsonify({"status": "ok", "service": "DermaStratif API"})
...
def predict():
    ...
    try:
        ...
        if not is_valid_skin_image(image_array):
            return jsonify({"error": "The uploaded image does not appear to be a valid skin image. Please upload a clear, close-up photograph of a skin lesion."}), 422
        ...
        if analysis is None:
            log.error("[REQUEST] Both vision analysis and fallback failed — returning error")
            return jsonify({"error": "We were unable to analyze this image. Please try again with a clearer, well-lit photograph of the skin lesion."}), 500

        if analysis.get("not_skin_image", False):
            log.warning("[REQUEST] Image rejected as non-skin by vision analysis")
            return jsonify({"error": "The uploaded image does not appear to show a skin lesion. Please upload a clear, close-up photograph of the area of concern."}), 422
        ...
        log.info(f"[REQUEST] Returning JSON result — disease: {predicted_disease}, confidence: {confidence}, cancer: {is_cancer}")
        ...
        return jsonify({
            ...
            "severity": sanitize_display_text(analysis.get("ai_severity", "Moderate")),
            ...
            "disease_image": basic_info.get("image", "melanoma.png"),
        })
